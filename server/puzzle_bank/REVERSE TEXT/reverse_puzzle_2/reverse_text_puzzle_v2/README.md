# Simple Reverse Text — College Puzzle (v2)
...
```
python verifier.py answers.txt
```
...
- The **first character** of each decoded line (ignoring leading spaces) spells a final 10-character code when concatenated.
...
- `make_demo_answers.py` — generates `answers.txt` from the solution (organizer use only).
